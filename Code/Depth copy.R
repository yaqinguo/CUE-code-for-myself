...
data4$Sites=factor(data4$Sites)
data4$Paper.ID=factor(data4$Paper.ID)
data4 %>%
  group_by(Sites) %>%
  summarise(n=n())
data4 %>%
...
data4 %>%
  summarise(n_MBN=sum(!is.na(Mean_MBN) & Mean_MBN !="")/1139,
            n_MBP=sum(!is.na(Mean_MBP) & Mean_MBP !="")/1139,
            n_MBC=sum(!is.na(Mean_MBC) & Mean_MBC !="")/1139)
...
p_biome <- whittaker_base_plot()+
  theme_bw()+
  geom_point(data = data4, aes(x=MAT, y=MAP/10, color = depth_type), size=3, stroke=0.5, shape=19)+
...
Fig.1 <- ggarrange(p_biome, below_row, 
                  ncol = 1, 
                  labels = "a)", 
...
ggsave("figures/Fig1.1.tiff", width = 8, height = 10, dpi = 300)
...
map1
ggsave("figures/Fig_map.tiff", width = 7.5, height = 4, dpi = 300)
