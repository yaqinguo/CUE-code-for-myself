...
rf  <- rfe_rf_sub$results[,c("Variables","RMSE")]


p1.1 <- ggplot(rf, aes(x=Variables, y=RMSE, group = 1))+
  geom_line()+
  geom_point()+
  geom_point(data = subset(rf, Variables==11),
...
svm_data  <- rfe_svm_sub$results[,c("Variables","RMSE")]
...
p31 <- ggplot(data=rfe_xgb_sub, metric="RMSE") + theme_bw() + 
  ...
  theme(axis.title = element_blank())
p31
xgb_data  <- rfe_xgb_sub$results[,c("Variables","RMSE")]

p3.1 <- ggplot(xgb_data, aes(x=Variables, y=RMSE, group = 1))+
  geom_line()+
  geom_point()+
  geom_point(data = subset(xgb_data, Variables==19),
             aes(x=Variables, y=RMSE),
             color="blue",
             size=4)+
