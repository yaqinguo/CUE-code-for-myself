...
#-----------------------------
train_data <- fread("training_data_topsoil.csv")
...
hyperparameter_list <- readRDS("best_XGBoost_params_list_top.rds")  # loads object: hyperparameter_list
...
for(i in seq_along(hyperparameter_list)) {
  ...
  temp_task <- TaskRegr$new(id = paste0("global_pred_", i),
                            backend = data.table(pred_input, CUE =as.numeric(NA)),
                            target = "CUE")
...
# 9. Save rasters
#-----------------------------
writeRaster(mean_raster,  "CUE_mean_prediction_0.25deg_top.tif",  overwrite = TRUE)
writeRaster(lower_raster, "CUE_lower95CI_0.25deg_top.tif", overwrite = TRUE)
writeRaster(upper_raster, "CUE_upper95CI_0.25deg_top.tif", overwrite = TRUE)

#-----------------------------
# 10. Quick plot
#-----------------------------
plot(mean_raster, main = "Predicted Topsoil CUE (Natural Lands Only)")
